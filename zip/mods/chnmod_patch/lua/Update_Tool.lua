...
_G.UpdateThisModChnPatch = _G.UpdateThisModChnPatch or {}
UpdateThisModChnPatch._ModPath = UpdateThisModChnPatch._ModPath or ModPath
UpdateThisModChnPatch._SavePath = UpdateThisModChnPatch._SavePath or SavePath .. "UpdateThisModChnPatch.txt"
UpdateThisModChnPatch.Settings = UpdateThisModChnPatch.Settings or {}
UpdateThisModChnPatch.Caller = string.sub(debug.getinfo(1,'S').source,2)
UpdateThisModChnPatch.Time2Delay = Application:time() + 5
UpdateThisModChnPatch.Special_List = {}

function UpdateThisModChnPatch:Save()
...
local UpdateThisModChnPatchLog = function(txt)
	log('[UpdateThisModChnPatchLog] ' .. tostring(txt))
end

function UpdateThisModChnPatch:Load()
...
function UpdateThisModChnPatch:Add(data)
	self.Time2Delay = Application:time() + 3
	self.Settings[data.mod_id] = data.data
	self:Save()
end

function UpdateThisModChnPatch:Remove(data)
	self.Time2Delay = Application:time() + 3
	self.Settings[data.mod_id] = nil
	self:Save()
end

function UpdateThisModChnPatch:CompareVersion()
	self:Load()
		...
		if type(data.modworkshop_id) == 'number' then
			...
			for i, mod in pairs(BLT.Mods.mods) do
				...
				if _dump:find(mwsIDs) then
					thisMod = mod
					inx = i
					UpdateThisModChnPatch.Special_List[Idstring(mwsIDs):key()] = true
					UpdateThisModChnPatch.Special_List[Idstring(thisMod:GetId()):key()] = true
					if thisMod then
						...
						if not BLT.Downloads.clbk_dwnld_fin_no_ver then
							function BLT.Downloads:clbk_dwnld_fin_no_ver(data, http_id)
								...
								UpdateThisModChnPatchLog(string.format("[Downloads] Finished download of %s (%s)", download.update:GetName(), download.update:GetParentMod():GetName()))
								...
								local save = function()
									...
									cleanup()
									UpdateThisModChnPatchLog("[Downloads] Saving to downloads...")
									...
									UpdateThisModChnPatchLog("[Downloads] Extracting...")
									download.state = "extracting"
									wait()
									unzip(file_path, temp_install_dir)
									wait()
									UpdateThisModChnPatchLog("[Downloads] Going on unverified...")
									UpdateThisModChnPatchLog("[Downloads] Removing old installation...")
									wait()
									SystemFS:delete_file(install_path)
									local move_success = file.MoveDirectory(extract_path, install_path)
									if not move_success then
										UpdateThisModChnPatchLog("[Downloads] Failed to move installation directory!")
										download.state = "failed"
										cleanup()
										return
									end
									UpdateThisModChnPatchLog("[Downloads] Complete!")
...
function UpdateThisModChnPatch:Loop()
	DelayedCalls:Add('DelayedMod_UpdateThisModChnPatch_Loop', 3, function()
		if UpdateThisModChnPatch.Time2Delay > Application:time() then
			UpdateThisModChnPatch:Loop()
		else
			UpdateThisModChnPatch:CompareVersion()
		end
	end)
end


Hooks:Add('MenuManagerOnOpenMenu', 'UpdateThisModChnPatch_RunInitNow', function(self, menu, ...)
	if menu == 'menu_main' then
		UpdateThisModChnPatch:Add({
			mod_id = Idstring('新版收获日2贴吧联合汉化'):key(),
			data = {
				modworkshop_id = 9999999999,
				dl_url = 'http://pd2.2ymagic.com/pd2/chnmod_patch.zip',
				info_url = 'http://pd2.2ymagic.com/pd2/mod.txt',
				main_url = 'https://coding.net/u/Dr_Newbie/p/chnmod_patch'
			}
			-- dl_url = 'https://pd2chn.000webhostapp.com/download/chnmod_patch.zip',
			-- info_url = 'https://pd2chn.000webhostapp.com/download/mods/chnmod_patch/mod.txt',
			-- main_url = 'https://coding.net/u/Dr_Newbie/p/chnmod_patch'
		})
		UpdateThisModChnPatch:Loop()
	end
